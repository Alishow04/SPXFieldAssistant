...
function applyPermissions() {
  ...
  const adminElements = [
    ...
    document.getElementById("nav-pricing"),
  ...
  if (!canSeeAdmin && (currentPage === "page-kits" || currentPage === "page-config" || currentPage === "page-pricing")) {
...
function handleRealtimeError(collectionName, err) {
  console.error(`Erro no listener em tempo real (${collectionName}):`, err);

  // Não derruba o login. O usuário pode continuar usando os dados carregados pelo bootstrap(),
  // enquanto as regras do Firestore são ajustadas.
  if (typeof window.toast === "function") {
    window.toast(`Sem permissão para sincronizar ${collectionName} em tempo real.`, "error");
  }
}
...
function startRealtime() {
  ...
  realtimeUnsubscribers.push(
    onSnapshot(
      collection(db, COL_VISITS),
      snap => {
        window.visits = snap.docs.map(d => ({ ...d.data(), id: d.id }));
        renderCurrentPage();
      },
      err => handleRealtimeError(COL_VISITS, err)
    )
  );

  realtimeUnsubscribers.push(
    onSnapshot(
      collection(db, COL_EQUIPS),
      snap => {
        window.equipments = snap.docs.map(d => ({ ...d.data(), id: d.id }));
        renderCurrentPage();
      },
      err => handleRealtimeError(COL_EQUIPS, err)
    )
  );

  realtimeUnsubscribers.push(
    onSnapshot(
      collection(db, COL_KITS),
      snap => {
        window.kits = snap.docs.map(d => ({ ...d.data(), id: d.id }));
        renderCurrentPage();
      },
      err => handleRealtimeError(COL_KITS, err)
    )
  );
}
...
window.fsSubscribe = (col, callback) => {
  return onSnapshot(
    ...
    err => {
      console.error(`Erro no listener da coleção ${col}:`, err);
      if (typeof window.toast === "function") {
        window.toast(`Sem permissão para sincronizar ${col}.`, "error");
      }
    }
  );
};
...
onAuthStateChanged(auth, async (user) => {
  ...
  try {
    ...
    console.error("Validação do perfil do usuário:", err);
  ...
  // Carregamento inicial e listeners ficam fora do catch de autenticação.
  // Assim, uma regra bloqueando spx_visits/spx_equips/spx_kits não desloga o usuário.
  await bootstrap();
  applyPermissions();
  startRealtime();
