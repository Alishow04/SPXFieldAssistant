...
// Catálogo compartilhado pela Folha de Dados e pelos cards das visitas.
...
const FD_IMG_MAP = {
  ...
  // ── Bombas centrífugas APV ──
  'W+':   SPX_IMG_BASE + 'apv-pumps-centrifugal-wplus-iec-35-35-side-angle-view.png',
  'WS+':  SPX_IMG_BASE + 'apv-pumps-centrifugal-wsplus-iec-20-15-model.png',
  'WI+':  SPX_IMG_BASE + 'apv-pumps-centrifugal-wiplus-nema-front-angle-view.png',

  // ── Waukesha Cherry-Burrell ──
  '200 SERIES':  SPX_IMG_BASE + 'waukesha-cherry-burrell-pumps-centrifugal-s200-with-drain-front.png',
  'S200 SERIES': SPX_IMG_BASE + 'waukesha-cherry-burrell-pumps-centrifugal-s200-with-drain-front.png',
  'W68':         SPX_IMG_BASE + 'waukesha-cherry-burrell-valves-single-seat-w68w88.png',
  'W88':         SPX_IMG_BASE + 'waukesha-cherry-burrell-valves-single-seat-w68w88.png',
  'U1':          SPX_IMG_BASE + 'waukesha-cherry-burrell-pumps-positive-displacement-universal-1-open.png',
  'U2':          SPX_IMG_BASE + 'waukesha-cherry-burrell-pumps-positive-displacement-lobe-open.png',
  'U3':          SPX_IMG_BASE + 'waukesha-cherry-burrell-pump-pd-u3-01.png',
  'C SERIES':    SPX_IMG_BASE + 'waukesha-cherry-burrell-pumps-centrifugal-c-series.png',

  // ── Trocador de calor ──
  'TROCADOR DE CALOR': SPX_IMG_BASE + 'apv-mira-013-20mm-alta-alternate-feet.png',
};

const FD_IMG_FALLBACK = {
  VALVULA:  SPX_IMG_BASE + 'apv-valve-collage-2.png',
  BOMBA:    SPX_IMG_BASE + 'apv-pumps-collage.png',
  TROCADOR: SPX_IMG_BASE + 'apv-mira-013-20mm-alta-alternate-feet.png',
};

function fdNormalizeImageKey(value) {
  return String(value || '')
    .toUpperCase()
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
...
function fdImageCandidates(dados = {}) {
  return [
    dados.bombaModeloSpx,
    dados.modelo,
    dados.valveTipo,
    dados.tipoValvula,
    dados.descricao,
    ...
    .map(fdNormalizeImageKey)
    .filter(Boolean)
    .flatMap(value => {
      const withoutPrefix = value.replace(
        /^(VALVULA|BOMBA|TROCADOR DE CALOR|TROCADOR)\s*[-–—:]?\s*/i,
        ''
      );
      return withoutPrefix && withoutPrefix !== value
        ? [value, withoutPrefix]
        : [value];
    });
}

function fdImageKeyMatches(candidate, key) {
  if (candidate === key) return true;

  // Famílias que normalmente recebem números/sufixos de tamanho ou execução.
  if (/^(SW|SV|SVS|BLV|VPN|RUF|RG|D4|SD|DE|DA|UF|W68|W88)$/.test(key)) {
    return candidate.startsWith(key);
  }

  // Séries com "+" costumam aparecer como W+22/20, Ws+30/30 etc.
  if (key.endsWith('+')) return candidate.startsWith(key);

  // U1/U2/U3: aceita tamanho separado por hífen, barra ou espaço, sem confundir U1 com U10.
  if (/^U[123]$/.test(key)) {
    return new RegExp(`^${key}(?:$|[^0-9])`).test(candidate);
  }
...
function obterImagemEquipamento(dados = {}, usarFallback = true) {
  if (dados.imagem) return dados.imagem;

  const candidates = fdImageCandidates(dados);
  const keys = Object.keys(FD_IMG_MAP).sort((a, b) => b.length - a.length);

  for (const candidate of candidates) {
    for (const key of keys) {
      if (fdImageKeyMatches(candidate, key)) return FD_IMG_MAP[key];
    }
  }

  if (!usarFallback) return '';

  const typeText = fdNormalizeImageKey([
    dados.tipo,
    dados.tipoValvula,
    dados.valveTipo,
    dados.descricao,
  ...
  if (typeText.includes('TROCADOR')) return FD_IMG_FALLBACK.TROCADOR;
  if (typeText.includes('BOMBA')) return FD_IMG_FALLBACK.BOMBA;
  if (typeText.includes('VALVULA')) return FD_IMG_FALLBACK.VALVULA;

  return '';
}

function obterImagemFD(dados) {
  return obterImagemEquipamento(dados, false);
}

// Expor o mesmo catálogo para outras telas do SPX Field Assistant.
window.SPX_EQUIPMENT_IMAGES = Object.freeze({ ...FD_IMG_MAP });
window.obterImagemEquipamento = obterImagemEquipamento;
